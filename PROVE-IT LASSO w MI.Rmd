...
bootstrap_validation_logreg <- function(data, response, app_cv_fit, app_best_lambda, n_bootstrap) {
  ...
  for(i in 1:n_bootstrap) {
    ...
    #app_fit_best <- glmnet(data, response, alpha=1, family="binomial", lambda=app_best_lambda,  nlambda = 101, intercept = TRUE, exact = FALSE)
   ...
   # boot_fit_best <- glmnet(bootstrap_data, bootstrap_response, alpha=1, family="binomial", lambda=boot_best_lambda, intercept = TRUE)
...
for (i in 1:number_df) {
  # Assuming mimic_imp is a list of data frames
  data_i <- complete(mimic_imp, action = i)
