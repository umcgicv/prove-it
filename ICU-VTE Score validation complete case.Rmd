...
external_validation_1 <- ggplot(mimic_prob, aes(x = prob_vte, y = status_dich)) +
 geom_smooth(color = "darkblue", method = "gam", se = FALSE, size = 0.5) + # LOESS smooth line
  geom_smooth(color = "darkred", method = "lm", se = FALSE, size = 0.5) + # LM smooth line
  ...
  geom_abline() + # Adds a line with a slope of 1 and intercept of 0 by default
  xlab("") +
...
external_validation_2 <- ggplot(mimic_prob, aes(x = prob_vte)) +
  ...
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 0), color = "black", size = 0.05) # Add manual border for x-axis from x=0 to x=0.25


legend_plot <- ggplot(data.frame(x = c(0, 1), y = c(0, 1)), aes(x, y)) +
  geom_line(aes(color = "Linear Regression"), size = 0.5) +
  geom_line(aes(color = "Local Polynomial Regression"), size = 0.5) +
  ...
  scale_color_manual("", 
                     values = c("Linear Regression" = "darkred", 
                                "Local Polynomial Regression" = "darkblue",
